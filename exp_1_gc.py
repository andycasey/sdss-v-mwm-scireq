...
if not os.path.exists(results_path):
    ...
    with open(results_path, "wb") as fp:
        pickle.dump((
            vacuum_wavlengths, gc_candidates, gc_candidate_flux, 
...
for field_name in set(gc_candidates["FIELD"]):


    mask = gc_candidates["FIELD"] == field_name
    ...
    #           * (feh < -1.4)
    membership = np.zeros(sum(mask), dtype=bool)
    ax.scatter(
    ...
    fig = plot_globular_cluster_abundances(model, gc_candidates[mask],
        test_labels[mask], membership)
