...
restricted_model.train(**train_kwds)
restricted_model.write(os.path.join(OUTPUT_PATH, "restricted.model"), overwrite=OVERWRITE)


# Plot theta.
fig = tc.plot.theta(restricted_model, indices=np.arange(len(label_names) + 1))
...
# Do one-to-one.
oto_labels, oto_cov, oto_meta = restricted_model.test(
    training_set_flux, training_set_ivar, **test_kwds)

fig = tc.plot.one_to_one(restricted_model, oto_labels)
...
model.write(os.path.join(OUTPUT_PATH, "unrestricted.model"), overwrite=OVERWRITE)
...
fig.tight_layout()
fig.subplots_adjust(hspace=0, wspace=0)
fig.savefig(os.path.join(OUTPUT_PATH, "unrestricted_theta.pdf"), dpi=300)
...
fig.set_figheight(40)
fig.subplots_adjust(wspace=0, hspace=0)
fig.savefig(os.path.join(OUTPUT_PATH, "unrestricted_one_to_one.pdf"), dpi=300)
